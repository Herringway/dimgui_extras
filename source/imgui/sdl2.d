...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event)
{
    ...
    switch (event.type)
    {
        case SDL_MOUSEMOTION:
        {
            ...
            io.AddMousePosEvent(cast(float)event.motion.x / scaleX, cast(float)event.motion.y / scaleY);
            return true;
        }
        case SDL_MOUSEWHEEL:
        {
...
static if (SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE) {
    ...
    if (is_app_focused)
    {
        ...
        if (bd.MouseCanUseGlobalState && bd.MouseButtonsDown == 0)
        {
            ...
            io.AddMousePosEvent(cast(float)(mouse_x_global - window_x) / scaleX, cast(float)(mouse_y_global - window_y) / scaleY);
        }
    }
}
